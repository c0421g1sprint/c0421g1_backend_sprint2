...
public class FoodAndDrinkController_getTopFiveNew {
    @Autowired
    ...
    @Test
    public void getTopFiveNew_5() throws Exception {
        this.mockMvc.perform(
                MockMvcRequestBuilders.get(
                        "/api/food-and-drink/find-top-five-new"))
                .andDo(print())
                .andExpect(status().is4xxClientError());
    }

    @Test
    public void getTopFiveNew_6() {
        ResponseEntity<List<FoodAndDrink>> responseEntity
                = this.foodAndDrinkController.getTopFiveNew();
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());

        List<FoodAndDrink> foodAndDrinkList = responseEntity.getBody();

        Assertions.assertEquals(5, foodAndDrinkList.size());
        Assertions.assertEquals("FAD-0018", foodAndDrinkList.get(0).getFadCode());
    }
}
